...
**Verify it actually worked** — this step has silently failed before without
anyone noticing, because the Lambda's own error handling returns a normal
`{"success": false, "error": "..."}` payload (not a Lambda invocation error),
and the MCP tool call still reports back to the model as `"status": "success"`
even though the tool itself failed. Check CloudWatch directly:

```bash
aws logs tail /aws/lambda/zdl-tools-handler --region "$REGION" --since 10m
# Look for "codeProxyRefusedConnection: connection refused" — if present, the
# allowlist entry is missing, wrong, or has rotated. A clean run shows
# "COCKROACH_URL resolved from Secrets Manager." with no connection errors
# following it.
```
...
MCP tool calls are routed to it. The `inlinePayload` carries the 6 tool schemas
(the live gateway target already has all 6 registered; re-run this only if you
need to update the schemas or rebuild the target from scratch).

**Tool name gotcha:** the gateway invokes the Lambda with the tool name at
`context.client_context.custom["bedrockAgentCoreToolName"]` — **not** a
top-level `context.bedrockAgentCoreToolName` attribute (AWS's own examples are
misleading here). The separator observed at runtime between `<target_name>`
and `<tool_name>` is a *triple* underscore (`___`), not the `__` shown in most
docs/blog posts. `backend/lambda_handler.py` handles both by matching the
known tool names directly rather than assuming a fixed delimiter — if you
fork this handler, keep that pattern rather than re-introducing a
delimiter-based split, since it silently breaks every tool call
("Unknown tool ''" in the Lambda logs, with the tool call itself still
reporting `"status": "success"` back to the model).

```bash
aws bedrock-agentcore-control update-gateway-target \
  --gateway-identifier "$GATEWAY_ID" \
  --name "zdl-tools-handler" \
  --description "ZeroDayLib tool contracts: finding, policy, timeline, memory KNN search + store, apply_patch remediation" \
        ...
        "toolSchema": {
          "inlinePayload": [
            ...
            },
            {
              ...
              "description": "Apply the governance-approved remediation for a finding (simulated: mutates the application data model only, not real infrastructure). SAFETY: the patch is applied only when the finding'\''s decision_state is '\''allow'\''; findings under manual_review or deny are refused. On success the finding status becomes '\''remediated'\'' and a REMEDIATION_EXECUTED audit event is appended. Idempotent: a finding already '\''remediated'\'' returns success without re-applying.",
              "inputSchema": {
                "type": "object",
                "properties": {
                  "finding_id": {"type": "string", "description": "UUID of the finding to remediate. Must be a finding whose governance decision_state is 'allow'.", "format": "uuid"},
                  "action_id": {"type": "string", "description": "Optional idempotency hint for the remediation action. If omitted, it is taken from the finding's governance proposal (proposal_jsonb.action_id).", "maxLength": 255}
                },
                "required": ["finding_id"]
              }
            }
...
All 4 AgentCore runtimes (`zdl_supervisorAgent`, `zdl_ingestAgent`,
`zdl_governanceAgent`, `zdl_remediationAgent`) are declared in
`agentcore/agentcore.json` and deployed to the `default` target via
`agentcore deploy` — no additional steps needed for a fresh clone that already
has AWS credentials configured:

1. Run `agentcore validate` to confirm the schema is clean.

2. Run `agentcore deploy --target default -y` to provision/update all 4
   runtimes via CDK (non-interactive; use `--diff` first to preview).

3. After deploy, `agentcore status` shows each runtime's ARN and `READY`
   state; `agentcore/.env.local` is updated with the new runtime endpoint URLs.

The memories and gateway are consumed as external connections (by ARN) — CDK does
not touch or recreate them. `zdl_remediationAgent` reuses a pre-existing memory
resource (`harness_zdl_remediation_beb1-44e0wR7Mem`) via a `type: memory`
connection in `agentcore.json`, the same pattern used by the other 3 agents.

The `aws-targets.json` file also has a `prod` target defined for a
future promotion path; it is not currently used by any deploy step.
